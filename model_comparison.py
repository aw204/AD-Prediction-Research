...
cohort, using 5-fold stratified cross-validation repeated 10 times (50 runs), at
...
fixed 80 epochs in all conditions, and each cross-validation held-out part is scored once at the end.
"""
...
NUM_SPLITS = 5
NUM_REPEATS = 10  # 5 splits x 10 repeats = 50 total cross-validation runs
NUM_EPOCHS = 80  # Fixed
...
def fit_scaler(x_train):
    # Compute mean and standard deviation strictly on the training part to prevent data leakage
...
class FusionNet(nn.Module):
    """
    PyTorch supporting both Early and Late fusion,
...
def train_nn(fusion_type, encoding_type, x_train, y_train, sample_weights, x_test, y_test):
    ...
    # Train a fixed number of epochs. The cross-validation held-out part is scored once at the end,
...
def run():
    ...
    for name, (model_family, model_or_fusion, encoding_type) in models.items():
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(cross_validator.split(X_real, y_real)):
            ...
            # Fit continuous feature standardization on the real training part only
    ...
    print("Each cell is mean (SD) of AUC across the 50 cross-validation runs.")
    print(f"{'Model':<28}{'0x AUC':>16}{'3x AUC':>16}{'5x AUC':>16}{'7x AUC':>16}{'10x AUC':>16}{'Max Uplift':>12}")

    summary = []
    for name in models:
        mean_0x,  sd_0x  = results[name][0].mean(),  results[name][0].std()
        mean_3x,  sd_3x  = results[name][3].mean(),  results[name][3].std()
        mean_5x,  sd_5x  = results[name][5].mean(),  results[name][5].std()
        mean_7x,  sd_7x  = results[name][7].mean(),  results[name][7].std()
        mean_10x, sd_10x = results[name][10].mean(), results[name][10].std()
        ...
        cell = lambda m, s: f"{m:.3f} ({s:.3f})"
        print(f"{name:<28}"
              f"{cell(mean_0x, sd_0x):>16}"
              f"{cell(mean_3x, sd_3x):>16}"
              f"{cell(mean_5x, sd_5x):>16}"
              f"{cell(mean_7x, sd_7x):>16}"
              f"{cell(mean_10x, sd_10x):>16}"
              f"{uplift:>+12.3f}")
